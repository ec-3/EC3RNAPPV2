...
if (process.argv.indexOf('--android') > -1) {
  ...
  await uploadBuild(path.resolve(process.cwd(), 'android/app/build/outputs/apk/universal.apk'), `ec3-mobile-v${packageInfo.version}-b${packageInfo.build}-${timeLabel}.apk`)
...
if (process.argv.indexOf('--ios') > -1) {
  ...
  await runBuildIOS()
  await uploadBuild(path.resolve(process.cwd(), 'ios/dist/ec3.xcarchive'), `ec3-mobile-v${packageInfo.version}-b${packageInfo.build}-${timeLabel}.xcarchive`)
